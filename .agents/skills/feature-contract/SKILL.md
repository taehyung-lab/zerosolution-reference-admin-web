...
- List, filter, search, result state, sorting, or pagination: select the applicable sections of [references/list-workflow.md](references/list-workflow.md). A new list needs its complete lifecycle and composition; a result-only change still needs the Query and URL rules that govern that result.
