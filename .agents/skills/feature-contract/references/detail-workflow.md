...
아래는 역할별 배치 예시이며 필수 파일 목록이 아니다. 각 역할의 규칙은 위 절들이 소유한다.
현재 요청에 필요한 책임만 구현하고, 같은 소유권의 작은 책임은 함께 둘 수 있다. 없는 섹션·액션·이력을 빈 파일로 두지 않는다.
