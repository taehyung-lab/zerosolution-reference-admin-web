...
The consuming screen keeps upload/download scope, selected-versus-all meaning, format options, prerequisite validation, payload, permission, pending surface, retry, and post-success result. A list download is a selection-gated action ([list Selection and actions](list.md#selection-and-actions)); a form file is a `FormFileField` ([form Fields](form.md#fields)). Do not create a generic file controller because an issue omitted this wiring.

Read [mutations](../../api-contract/references/mutations.md) for payload and cache consequences.
