...
- **시나리오 확정됨**의 대조 범위 — 검색은 조건 조립·URL commit·요청 전 검증까지, 저장은 폼 검증·확인
  alert·검증된 업무 입력 조립과 요청 함수 호출까지. 상세는 그 화면이 가진 모든 최종 액션(발송·전체보기·변경·해지 같은
  행 액션, 팝업 열기/선택/반환, tab 이동, 인라인 폼 저장, 선택삭제)을 빠짐없이 포함한다.
- **시나리오 구현 완료**의 관찰 — 구현된 API 미연결 액션을 브라우저에서 최종 확인으로 눌러 업무별 요청 함수
  호출과 그 `console.log`를 관찰한다. 검색·이동·팝업 선택 같은 내부 전이는 실제 URL과 화면 상태로 확인한다.
...
- 호출부는 업무 이름이 있는 함수를 연결한다. 여러 화면에서 같은 업무를 호출하면 같은 함수를 쓰되,
  레코드 ID·자식 레코드 ID·검색 범위처럼 실제 요청에 필요한 정체성을 전달한다. 서로 다른 업무를 범용 dispatcher로 합치지 않는다.
...
- route 가 여는 작성 다이얼로그처럼 여러 route가 같은 요청을 소비해도 최종 callback은 필수다. 각 호출부에서 업무별 요청
...
시나리오 목록에는 등록·수정 외에도 발송·인라인 저장·일괄변경·삭제·다운로드 등 현재 surface의 최종 액션과
