...
Segment names are the product's external URL contract and need not match the screen folder name ([folder structure](../../folder-structure-contract/SKILL.md)). A resource collection defaults to the plural noun. A state, workflow, or fixed view takes the name the product's IA gives it, which is often not a plural. A URL the product has already fixed wins over both defaults.
