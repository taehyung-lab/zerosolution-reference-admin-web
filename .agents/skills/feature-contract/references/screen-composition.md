...
- 목록의 URL 필드·codec·전이 정책·액션 요청 경계는 [list-workflow 형태](list-workflow.md#형태)와 해당 행동 계약이 소유한다. 필요한 역할은 요청과 실제 데이터·상태 흐름으로 고른다. 기존 소비자는 채택·이탈의 검증 자료이지 필수 파일이나 제품 정책을 정하는 기준이 아니다. 액션이 없으면 요청 경계도 만들지 않는다.
