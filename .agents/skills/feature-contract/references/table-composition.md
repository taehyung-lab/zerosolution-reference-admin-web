...
A top-level list result uses kind C's query/render mechanics, but [list-workflow.md](list-workflow.md) owns its complete lifecycle and validated URL params. The local-params choice below applies to child lists, not that top-level route. Kind A is confirmed by the parent DTO alone. Kinds B, child C, and E require their endpoint, params, and failure semantics; child C also needs confirmed sharing/restoration policy. Stop that surface when those facts are missing instead of guessing an endpoint or URL prefix.
...
- Kinds C and E follow the list workflow's Query lifecycle: key and request use the same resolved params, never drafts. Top-level C keeps committed params in validated route search. Child C uses URL only for confirmed sharing/restoration, otherwise its nearest host; every E param stays local. Unconfirmed child sharing policy is a stop condition, not permission to guess local or URL ownership.
