...
화면 폴더 이름은 **항상** `{entity}-list`·`{entity}-detail`·`{entity}-form` 이다. `{entity}` 는 surface ID 도 route segment 도 model 타입 이름도 아니다 — **그 폴더 이름만 보고 무엇을 다루는 화면인지 읽히는 최소 한정자**이며 singular kebab-case 다. 기본 명사만으로 읽히면 그대로 쓰고, 그 명사가 무엇에 속한 것인지 물어야 읽히면 속한 대상을 앞에 붙여 `{owner}-{noun}-list` 로 쓴다. 읽히는가의 기준은 도메인 폴더 경로가 아니라 **폴더 이름 단독**이다 — 상위 폴더가 이미 말해 준다는 이유로 접두를 생략하지 않고, 도메인의 대표 엔티티라도 마찬가지다. model 타입·행 타입과 1:1 이 아니어도 되고, 한 `{entity}-list` 가 여러 URL 변형을 열 수도 있다. **한정자는 폴더 이름과 진입 `{Entity}ListScreen` 에만 붙인다** — 폴더 안의 파일은 그 폴더 안에서 읽히므로 짧은 이름을 쓴다(`screens/{owner}-{noun}-list/` 안에서 진입은 `ui/{Owner}{Noun}ListScreen.tsx`, 나머지는 `model/{noun}-list-search.ts`·`ui/{Noun}ListResult.tsx`). 한 목록의 URL 변형(탭·경로별 고정 필터)은 같은 `{entity}-list` 를 쓰고 `model/{entity}-list-definition.ts` 가 차이를 표현한다. route 가 여는 다이얼로그 하나도 그것을 여는 화면의 `ui/` 에 둔다. 별도 `pages` 레이어는 없다.
