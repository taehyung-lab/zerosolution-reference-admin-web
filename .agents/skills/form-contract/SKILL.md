...
폼 값은 TanStack Form, 저장 요청은 도메인 API, 목적지는 route가 소유한다. 아래 표의 `useSaveForm`
배선은 확인 → 저장 → 완료 확인 절차를 채택한 현재 조합이다. 다른 절차는 화면이 필요한 단위로
조립하되 입력 보존·오류 노출·기준선·이탈 보호의 책임을 빠뜨리거나 중복 소유하지 않는다.
...
아래는 확인 → 저장 → 완료 확인을 요구하는 폼의 현재 `useSaveForm` 사용법이다. 제품의 확인 여부·
완료 표현·목적지는 fact나 사용자 요구로 정한다. 다른 절차면 필요한 하위 단위로 조립하며 기존 훅에
업무 mode를 늘리지 않는다. `onServer` 오류 배치·입력 보존·기준선 갱신은 변경 시 확인할 불변식이다.
...
- 미연결 mutation은 제품이 확인한 성공 경로를 시나리오로 관찰한다. 확인되지 않은 목적지나 완료
  표현을 이 훅의 기본 절차로 확정하지 않는다. 실제 저장 여부는 별도 미확인이다.
...
`useSaveForm`을 채택한 폼의 배선과, 필드 변경 시 확인할 책임이다.
...
- 조건부 필드군: 제품이 숨긴 입력의 복원을 요구하면 값은 부모 폼에 두고 가시성만 바꾼다. schema 가
  authoritative 상태로 검증하고 mapper가 확정 입력만 싣는다. 폐기를 요구하면 값을 명시적으로
  비운다. 정책이 없으면 보존·폐기 중 하나를 제품 사실로 단정하지 않는다.
- 반복 행은 배열 필드 어댑터 + 표 primitive. row factory·stable id·최소 개수는 그 화면이 정한다.
  **배열 index 를 행 정체성으로 쓰지 않는다** — 삭제 뒤 오류가 다른 행으로 옮겨간다.
  ...
  복제하지 않는다.
- 입력 단계 차단을 요구하면 `maxLength`로 막고 schema도 상한을 검증한다. 상한·오류 표현은
  fact에서 확인한다. 문구가 기록되지 않았다는 사실만으로 오류를 표시하지 않는 정책을 만들지 않는다.
- blur 에서도 알려야 하는 불일치는 `validators.onBlur` 에 필드 하나만 더한다.

## 취소와 이탈

아래는 현재 guard 조합의 동작이다. 보호 대상과 확인 여부는 제품 요구로 정하며, 이 절차를 채택한
폼에서 적용한다.
...
아래에서 바뀐 동작과 영향을 받는 연결을 고른다. 확인·완료 절차는 해당 폼이 채택한 경우에만 본다.
...
브라우저 증거는 **어떤 값을 넣어 무엇을 눌렀고 어디로 갔는지**를 적는다. E2E를 추가할지는
[변경 영향과 재발 위험](../../../AGENTS.md#전역-완료-기준)으로 정한다. 동작 단언은 제품 요구에 근거한다.
