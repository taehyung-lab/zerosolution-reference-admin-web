...
돌아가는지를 정한다. 이 절차가 보장하려는 것은 셋이다(2026-09-11 사용자 확정): **① 구조는 역할별 형태 절이 정한
공용 형태로**(N4), **② 시나리오는 요청 문장의 사용자 요구가 있으면 그것을, 없으면 확정 답과 Figma·Notion
원장·원문을, 원장이 없거나 낡음·충돌·변경 징후가 있으면 실측을 따라**(N2), **③ 구현이 그대로 반영됐는지 요구사항별로 검증**(N5·N6). **규칙을
