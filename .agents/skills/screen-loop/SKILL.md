...
**진입·실행·후속 이동·복구의 최소 연결 범위**를 정한다. 성공 조건은 현재 요청의 명시적 지시,
이전의 확정된 결정, 해당 원장에서 읽는다 — 명시적 지시가 이전 결정보다 우선하고 충돌이
불명확하면 확인한다. 기존 연결은 재사용하고, 필요한 연결이 없으면 확인된 요구만 구현한다.
새로운 제품 동작이나 독립 workflow가 필요한 부분은 범위 변경을 제시하고 확인하며, 확인을
받지 못하면 **그 부분만** 보류하고 보고에 남긴다. 확인된 요구를 구현하기 위한 기술적 판단까지
보류하지 않는다.

경계는 파일·route의 인접성이 아니라 관측 가능한 성공 조건이 정한다.

| 연결을 빼면 | 판정 |
| --- | --- |
| 요청한 흐름을 수행할 수 없다 | 범위에 포함 |
| 요청한 흐름이 그대로 성립한다 | 제외 |
| 연결하려면 새 제품 정책·화면 기능을 결정해야 한다 | 자동 구현하지 않고 범위 확인 |

**slice 는 작업을 나누는 단위이지 완료 조건을 잘라내는 단위가 아니다.**
