...
요청이 여러 종류에 걸쳐도 임의로 하나만 남기지 않는다. 요청한 성공 조건에 필요한 종류의 계약을 모두 읽는다.
...
| --- | --- | --- | --- | --- |
| **1 요청 고정** | 요청 본문. 루트 기준은 [AGENTS.md](../../../AGENTS.md)이며 런타임이 주입하지 않았을 때만 연다 | 요구 번호, 관측 가능한 성공 조건, 현재 흐름·영향 범위, 편집 후보, 검증 방법. 새 요구나 범위 변경은 기존 번호에 섞지 않고 번호를 새로 딴다 | 2 | 요청 오해·새 요구 → **1** |
...
- 결과가 `render`인 요구를 앱에서 도달 가능한 실제 route나 host에서 열지 못했으면 그 요구는 `미구현`이다. 미연결 사실을
