...
`src/shared` 와 `src/api` 가 지금 내보내는 공용 단위의 계약이다. 한 줄이 한 단위이고, 열은 **caller 가 넘기는 것 / 단위가 소유하는 것 / 소유하지 않는 것**이다. 표에 없는 prop·mode·callback 을 한 호출자 때문에 더하지 않는다([promotion](promotion.md)). 화면에서 어떤 단위를 언제 조립하는가는 [list](../../feature-contract/references/list.md)·[detail](../../feature-contract/references/detail.md)·[form](../../feature-contract/references/form.md)이 소유한다.
...
| `useSaveForm` | `schema`, `defaultValues`, `resetKey?`, `sections`, `save: { run, isPending, getDefaultValues? }`, `mapError`, `onDone` | 폼 인스턴스, 저장 단계, 잘못된 제출 reveal·focus, `onServer` 배치·정리, 기준선 갱신, dirty guard, `dialogs` 노드 | schema·mutation·오류 분류·목적지([form](../../feature-contract/references/form.md#save-lifecycle)) |
