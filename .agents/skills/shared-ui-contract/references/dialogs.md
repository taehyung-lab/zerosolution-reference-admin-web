...
- Copy moves to the `shared` namespace only after the promotion contract confirms the same sentence and interaction across consumers. A differing sentence stays feature copy with its product evidence.
- A form dialog that requires an explicit close affordance passes `Dialog.closeLabel`; the labelled × button uses the same controlled close request as Escape/outside dismissal. Section headings inside the dialog are plain headings, not `SectionCard`.
...
Dirty-close eligibility and consumer wiring are owned by [form-workflow.md](../../feature-contract/references/form-workflow.md#cancel-and-tabs).
