...
- The common alert catalogue (Figma `1.3 Alert` frames `1.1.3.1.x`/`1.1.3.2.x`, `docs/reference/zero-sol/01-common.md`) is `shared` namespace copy: `formSave.confirmDescription` (저장 확인), `unsavedChanges.description` (화면 이동), `bulkAction.confirm` (선택 변경), `deleteConfirm.description` (삭제 확인, 2026-09-11), and the 완료 sentences. A feature does not re-declare one of these sentences in its own namespace; a sentence that differs from the catalogue (`members:bulk.confirmDelete` "선택 항목을 삭제하시겠습니까?") stays feature copy with its ledger row. The manager and counsel delete confirmations moved to the shared key on the same day.
