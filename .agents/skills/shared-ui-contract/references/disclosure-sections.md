...
- A titled block of a detail/form/settings screen (an information block, an update-history block, a settings block): `SectionCard`. Collapsible blocks repeat the `^` header; a screen whose inventory draws fixed sections uses `collapsible={false}`.
...
- `errorCount` (0 hides it) renders as a danger `Badge` **inside the trigger**, so the header's accessible name becomes "<섹션 제목> 오류 2개" and stays announced while collapsed. Copy is `shared:formSection.errors`; the caller passes only the number. It is not a live region — the field `role="alert"` messages and the first-error focus already announce a rejected submit.
