...
- `FilterPanel({ title, collapseLabel, expandLabel, submitLabel, resetLabel, onSubmit(SubmitEvent<HTMLFormElement>), onReset, children })` — a `form` named by `title` with an uncontrolled disclosure (`aria-expanded`/`aria-controls`); the caller commits in `onSubmit` and chooses the `onReset` destination (`{}` for an explicit-search list, `{ searched: false }` for an immediate-load list that resets to idle). Input reset mechanics are described in [shared-values.md](shared-values.md).
