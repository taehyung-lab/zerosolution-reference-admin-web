...
- `useUnsavedChangesGuard({ when, refuseSilently? })` returns `{ dialog, leave, close }`. `leave(navigate)` uses the Router blocker; `close(discard)` confirms a local dismiss without pretending it is navigation. Both ask only when dirty, preserve input on cancellation, and use one rendered `dialog`. `refuseSilently` refuses either exit while pending. Route navigation uses `shared:unsavedChanges.*`; explicit cancel/local dismiss uses `shared:formCancel.*`. `UnsavedChangesGuard.tsx` remains the one shared file allowed to import the Router. The app mounts one `UnsavedChangesProvider` inside Router to aggregate dirty/pending facts and prevent sequential questions from concurrent forms; each hook still owns local close. `close(discard, { when: localDirty })` scopes cancellation when the caller has multiple forms. A standalone hook without the provider retains its own blocker. A dialog routes cancel, ×, Escape and outside dismissal through `close`; values and the discard callback remain caller-owned (2026-09-05 user decision).
