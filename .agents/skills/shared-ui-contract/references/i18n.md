...
- `usePeriodPresetLabels()` returns `Record<PeriodValue, string>` (8 presets + `CUSTOM`) from `shared:list.periodPresets`; callers map their adopted preset values onto it.
