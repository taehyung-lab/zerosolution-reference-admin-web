...
- `ResultSummary({ groups: { key, items: { key, text }[] }[] })` renders each completed sentence as a list item; empty groups disappear and separators are decorative. For a single total, `ResultTotal({ searched, total })` owns `shared:list.total`, locale number formatting and the absent-before-search/visible-zero distinction using this summary layout; multi-item summaries (발권: 총·발권대기·발권완료…) are extra groups with feature meaning.
