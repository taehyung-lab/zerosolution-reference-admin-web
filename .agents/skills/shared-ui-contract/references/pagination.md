...
- `Pagination({ page, totalPages, onPageChange, ariaLabel, previousLabel, nextLabel })` renders nothing when `totalPages <= 1` and shows `← Previous 1 2 3 … 67 68 Next →` (a window around the current page plus anchors). The window and Previous/Next boundaries clamp into range; `aria-current` marks only the raw `page`. Out-of-range input has no current marker and may expose recovery controls. Recovery policy belongs to [list-workflow](../../feature-contract/references/list-workflow.md#result-ownership), not this renderer.
