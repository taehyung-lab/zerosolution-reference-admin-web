...
단순 카피·스타일 작업은 요구사항·범위·최소 검증을 짧게 공개한다. 새 화면·workflow·API/shared 경계 변경은 기존 계약과 `채택 / 수정 / 제외`를 비교한 구현 체크포인트를 공개한다. 상세 항목과 준비 절차는 [에이전트 실행 안내](scripts/agents/README.md#prepare-before-editing)가 소유하며, 형식용 영구 계획 문서는 만들지 않는다.
...
- 이 저장소의 화면 작업은 CSS와 실서버 연결을 범위에서 뺀다. 검증 증거는 디자인 대조가 아니라 **인벤토리 대조**이며, 상태는 **시나리오 확정됨 → 시나리오 구현 완료 → 완료 → 이관 검증됨** 순이다. 각 상태의 관찰 범위와 액션별 실행 규칙은 [mutation-actions.md](.agents/skills/feature-contract/references/mutation-actions.md)가 소유한다. 실 API를 연결해 업무군 전체 workflow를 실측하면 **완료**, 해당 bundle을 `채택 / 수정 / 제외`까지 하면 **이관 검증됨**이다. 적용하지 않은 상태는 사유를 완료 보고에 적는다.
