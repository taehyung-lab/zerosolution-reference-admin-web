...
- 위임에는 목표, 범위, 소유 파일, 금지 사항, 기대 증거를 적으며 사용자와 상위 작업의 권한을 넘기지 않는다. 같은 파일이나 설계 결정을 여러 작성자에게 동시에 맡기지 않으며 최종 편집과 정합성 책임자는 하나다. 브리프를 파일로 둘 조건은 [실행 안내](scripts/agents/README.md#put-a-delegation-brief-in-a-file-only-when-it-must-outlive-the-message)가 소유한다.
