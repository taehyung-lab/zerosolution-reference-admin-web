...
| `pnpm contracts:check` | 규범 문서와 저장소 설정의 기계적 정합성. verify 체인 투영, `pnpm` 명령·로컬 link, 문서 notice와 `scripts/loop/baseline.json`이 소유하는 항상 로드되는 `AGENTS.md + description` 상한, 런타임 포인터·skill 어댑터, 이관 sentinel, API 이음매, seed 폐쇄, 이관 manifest를 검사한다. `--mode target`은 이관된 저장소용이다 |
