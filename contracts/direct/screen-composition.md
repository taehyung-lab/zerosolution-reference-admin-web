...
- A piece two screens of one domain share with identical meaning, transitions, and failure lifecycle moves to `mechanics/{capability}/{ui,model}` ([folder-structure](../contract/source-structure.md)). Screens never import sibling screens; ESLint enforces it.
...
- Cross-feature wiring (another domain's dialog on this screen) happens in the route ([router](route-composition.md#thin-route)).
...
조회·선택지·mock 책임은 [query-cache](../contract/server-state.md#서버-연결-전후의-책임), 공용 승격은 [promotion](shared-ui.md), 미연결 mutation 의 종착점은 [mutations](../contract/server-state.md#시나리오-요청)를 따른다. 대표 화면 하나의 적용을 다른 route 와 다이얼로그의 적용 완료로 간주하지 않는다.
