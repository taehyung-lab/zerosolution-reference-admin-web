...
설계 문서 §14는 Day 1에 OpenAPI pull/generate, transport, 인증, 실제 workflow, contract gate를
함께 검증하라고 요구한다. 동시에 `AGENTS.md` §3은 불확실한 서버 계약을 추측하지 말라고 한다.
백엔드가 없는 상태에서는 이 둘이 동시에 만족되지 않는다.

검토한 대안:

1. **프론트가 신규 스펙을 직접 저작** — 더 깔끔한 추측일 뿐이며, 상상한 것만 증명한다.
2. **백엔드가 나올 때까지 셸만 구성** — §13 기계 게이트를 실제로 만들 수 없다.
...
- 출처: `https://dev.api.zeroplussolution.com/v3/api-docs`
...
4. 이 계약으로 만든 코드는 공용 승격의 "두 번째 실제 사용" 카운트에 넣지 않는다 (설계 §9).
...
| # | 보정 | 규모 | 근거 |
| --- | --- | --- | --- |
| 1 | parameter `in` 대문자 → 소문자 | 325 | OpenAPI 3 스펙은 소문자만 허용 |
| 2 | 중복 `Authorization` header parameter 제거 | 256 | 전역 `security`와 중복. 제거하지 않으면 생성 시그니처에 토큰이 노출된다 |
| 3 | `$` 포함 component 키 정규화 (`$`→`_`) + 모든 `$ref` 재작성 | 470 | 스펙 패턴 `^[a-zA-Z0-9.\-_]+$` 위반. Orval 8이 거부 |
...
| 6 | 누락된 path parameter를 형제 operation 정의로 보충 | 2 | 같은 path의 `GET`이 이미 `id: integer/int64`로 선언. 형제 정의가 없으면 실패 |

불변식: schema 개수 불변(530), `$ref` 개수 불변(1145)이며 변환 후 전부 해석되어야 한다.
...
- **응답 코드 체계 (dev 서버 실측, 2026-08-27)**
  ...
  - **업무 실패: HTTP 200 + `{"header":{"resultCode":2100,"resultMessage":"NOT EXIST"}, ...}`**
    → HTTP 200만으로 성공을 판정하면 안 된다는 설계 §7의 근거가 실측으로 확인됐다
...
## 봉투 unwrap과 생성 타입의 정합 (필수 결정)

Orval은 endpoint의 반환 타입 인자로 **봉투 타입**(`Rs...` = `{header?, data?}`)을 넘긴다.
mutator가 봉투를 벗겨 `data`만 반환하면서 타입은 그대로 `T`라고 선언하면
**정적 타입이 런타임과 다른 거짓말**이 되고, 이는 `api-contract/openapi.md`의
cast/silent reshape 금지를 정면 위반한다.

결정: mutator의 반환 타입을 `UnwrapEnvelope<T>`로 매핑한다.

```ts
export type UnwrapEnvelope<T> = T extends { data?: infer D } ? Exclude<D, undefined> : T
```

이로써 `signIn(...)`의 정적 타입이 `Promise<AhAuthDTOSignInResponse>`가 되어 런타임과 일치한다.
cast나 `any`를 쓰지 않는다. 회귀는 `src/api/http/transport.test.ts`가 막는다
(`result.accessToken` 직접 접근이 봉투 타입이면 컴파일되지 않는다).

## 결과

- `pnpm api:check`가 네트워크 없이 실제로 통과한다 (validate → prepare → Orval → generated typecheck).
- **269개 operation**이 생성되어 전부 프로젝트 Axios 인스턴스(`customInstance`)를 경유한다.
  (`customInstance<` 문자열은 시그니처와 호출부에 각각 나타나 538회 등장하지만 operation 수는 269다.)
