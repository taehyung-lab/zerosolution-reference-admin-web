...
이 결정은 저장 위치, 재발급·재전송 경계, 동시 회전 수렴과 terminal 분류를 택한 이유만 보존한다.
현재 adapter·순서·직렬화·incident 표면은 실행 코드와 계약 검사가 소유한다.
...
당시 신규 계약과 현재 요구사항이 없던 부가 헤더·복제 지연·SSE 결합은 채택하지 않았다. 현재 제외 범위는
활성 계약과 코드에서 확인하며, 이 ADR이 파일 이름 목록을 유지하지 않는다.
