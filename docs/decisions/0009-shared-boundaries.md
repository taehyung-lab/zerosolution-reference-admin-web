...
- 날짜: 2026-08-28
- 근거 재확인: 2026-09-02 — Figma 55 page + Notion Feature 72페이지 전체 인벤토리로 재판정
...
Managers의 route schema, defaults, option query, endpoint, enum, params mapper, query key, result-state 결정, columns, summary 의미, page-size default·preset 선택, copy, permission, selection, bulk와 navigation은 feature-local이다. 리허설의 `manager-types`, `excludeInternal`, permission endpoint·누락 같은 사실을 제품 공통 계약으로 승격하지 않는다.
...
2026-09-02 전체 인벤토리는 provisional 근거를 강화했지만 코드 consumer는 여전히 Managers 하나다. 인벤토리에서 문장 단위로 반복이 확인된 행동(행 선택·일괄 변경 3단계 alert·다운로드 선택/전체·lookup·보기/정렬 마지막 값 기억)은 [판정 기록 §4](../reference/zero-sol-figma-analysis.md)에 후보로 두고, 이 ADR은 첫 코드 consumer가 생길 때만 단계를 부여한다. 팝업 안의 목록(게시물 > 작성자 검색)과 별도 창(현장발권)은 shared가 Router·shell을 모르는 현재 경계의 직접 근거이며, dialog 안 검색은 `table-composition.md` kind E가 소유한다.
