...
일반 작업은 흐름·소유권·재사용·단순성과 관련 계약의 `채택 / 수정 / 제외`를 필요한 길이로 설명한다. 네 개 JSON 필드는 기록 모드에서만 작성한다. 작성 방법은 [screen-loop의 설계 선택](../../.agents/skills/screen-loop/SKILL.md#설계-선택), 필드는 agents README가 소유한다. 문장을 채웠다는 사실은 설계 승인이나 의미적 타당성의 증거가 아니다.
