...
- 이 저장소의 시나리오는 API 호출에 필요한 입력이 확정되는 직전까지 쓴다. API·mock을 다루지 않으며
  ...
  제품의 계약과 검증이 소유한다. 2026-09-05 사용자 확정: 이 레퍼런스의 시나리오 완료는 검색·선택·
  검증·확인·취소를 실행해 호출 직전의 대상과 입력값까지 확인한 상태다. 문서 작성만으로 실행 검증을
  대신하지 않으며, 검증용 입력은 서버 계약과 구분한다. 현재 적용 현황은 각 카드에 기록한다.
