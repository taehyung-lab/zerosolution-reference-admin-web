...
- 시나리오의 상태 어휘와 정의는 [도달 상태](../../../AGENTS.md#도달-상태), 상태별 관찰 범위는 [server-state의 시나리오 상태와 관찰 범위](../../../.agents/skills/server-state/SKILL.md#시나리오-상태와-관찰-범위), API 연결 전 실행은 같은 계약의 [시나리오 요청](../../../.agents/skills/server-state/SKILL.md#시나리오-요청)이 소유한다. 각 카드는 적용 상태와 실제 관찰 증거를 기록하며, 문서 대조·요청 직전 실행·실제 서버 성공을 구분한다.
