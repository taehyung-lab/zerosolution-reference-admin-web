...
- 이 저장소의 “API 호출 직전”은 실제 HTTP 호출 전까지라는 뜻이다. 2026-09-06 사용자 확정:
  구현된 API 미연결 액션은 검색·선택·검증·확인·취소를 실행하고, 최종 확인 시 검증된 대상과 입력을
  업무별 요청 함수에 전달해 그 함수의 `console.log`까지 브라우저에서 관찰해야 **시나리오 구현 완료**다.
  내부 검색·이동·팝업 선택은 실제 URL·화면 상태로 확인한다. 문서와 인벤토리 대조만 끝난 상태는
  **시나리오 확정됨**이며 실행 검증을 대신하지 않는다. 요청 함수의 규칙은
  [mutation-actions.md](../../../.agents/skills/feature-contract/references/mutation-actions.md#api-연결-전-시나리오-요청)가 소유한다.
- 시나리오 구현 완료는 실제 API 성공을 뜻하지 않는다. 성공 응답·완료 alert·cache 갱신·실서버 동작은
  후속 제품의 계약과 검증이 소유한다. API·mock 구현과 브라우저 오류 주입은 이 입력 시나리오의 범위에서
  제외하며, 검증용 입력은 서버 계약과 구분한다. 현재 적용 현황은 각 카드에 기록한다.
