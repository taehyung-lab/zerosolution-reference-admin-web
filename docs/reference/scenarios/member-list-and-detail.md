...
| **F7** `[외부]` **목록 조회 실패가 "검색해주세요"로 위장**됐고, 다른 목록에서는 실패가 빈 결과로 보였다 | 요청 실패·재시도 사실이 결과 표면에 도달하지 않았다 | 검색 전·빈 결과·오류가 서로 다른 사실이고, 하나라도 표면에 도달하지 못하면 **남은 것 중 하나로 위장된다**는 것 |
| **F8** `[외부]` 빈 결과 문구가 "등록된 데이터 없음"과 "검색 결과 없음"으로 갈리지 않았고, 검색 전 판정 기준을 나중에 바꿔야 했다 | 검색 전과 검색 후 빈 결과의 판별 조건을 분리하지 않았다 | 검색 전/후의 판별자는 **제품이 지정한 실제 필터 값**이어야지 URL이 비었다는 사실이 아니라는 것 |
...
| **F10** `[외부]` **헤더 전체선택이 해제되지 않았다.** 선택 불가 행이 섞인 페이지에서 전체 checkbox가 늘 "전체선택"으로만 동작 | 행 checkbox와 헤더 checkbox가 서로 다른 선택 가능 기준을 사용했다 | 헤더 전체선택 상태가 **행의 선택 가능성에서 파생**되므로 그 판정의 소스가 하나여야 한다는 것 |
| **F11** `[외부]` 목록 행이 첫 렌더 상태에 **얼어붙어 상태가 바뀐 행의 체크박스가 잠겼고**, checkbox 1회 조작이 과도한 DOM 변이를 만들었다 | 행의 렌더 정체성이 행 밖의 선택 가능 조건을 포함하지 않았고 상위 입력도 불안정했다 | 선택 가능 여부가 **행 밖의 사실**에 의존하면 그 사실이 렌더 identity에 있어야 한다는 것 |
