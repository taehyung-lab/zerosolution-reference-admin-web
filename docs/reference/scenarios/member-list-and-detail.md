...
표기: `[확인]` 저장소 안 `파일:줄`이나 스키마 원문으로 확인한 것. `[추론]` 그 사실에서 끌어낸 판단. `[미확인]` 6절. 저장소 밖 근거는 2026-09-04에 비교 가능한 운영 어드민의 코드와 fix 커밋 본문·diff를 읽어 확정한 **런타임 실패 사실**만 `[외부]`로 쓰고, 그 제품의 설계·파일 구조·구현 선택은 증거에서 제외한다. 그쪽에서 이 계열에 대응하는 **살아 있는 화면은 회원이 아니라 운영자 계정 계열**이었고(회원 이름의 화면은 내비게이션에 없고 목 데이터만 쓰는 보일러플레이트 잔재였다), 따라서 `[외부]`는 목록·상세·폼이라는 **같은 축**에서 캔 것이지 회원 도메인 정책이 아니다.
...
네 화면이 공유하는 사용자 목표는 하나다. **조건으로 회원 모집단을 좁히고, 고른 대상에 작업을 하거나, 한 명을 열어 사실을 확인·정정한다.** 목록에서 상세로, 상세에서 폼으로 가는 동안 사용자가 이미 지불한 것(검색 조건·선택·입력)이 사라지지 않아야 한다는 것이 이 계열의 유일한 연속성 요구다. `[추론]`
...
`[계약]`은 우리 계약이 실측해 이미 기록한 실패, `[외부]`는 비교 입력의 fix 커밋에서 캔 것이다.
...
| **F7** `[외부]` **목록 조회 실패가 "검색해주세요"로 위장**됐고, 대시보드에서도 실패가 빈 결과로 보였다. E2E 감사에서야 드러났다 | 공용 목록 컨트롤러가 query error·retry를 결과 표면에 **전달하지 않았다** | 검색 전·빈 결과·오류가 서로 다른 사실이고, 하나라도 표면에 도달하지 못하면 **남은 것 중 하나로 위장된다**는 것 |
| **F8** `[외부]` 빈 결과 문구가 "등록된 데이터 없음"과 "검색 결과 없음"으로 갈리지 않았고, 이후 **"검색 전" 판정 기준을 URL에서 셀렉터 상태로 갈아엎어야** 했다 | 검색 여부를 URL의 비어 있음으로 유도했다 | 검색 전/후의 판별자는 **제품이 지정한 실제 필터 값**이어야지 URL이 비었다는 사실이 아니라는 것 |
| **F9** `[외부]` URL의 만료된 참조 ID가 옵션에서 안 잡히는 상태와 "이름 미등록"이 `-` 하나로 합쳐졌고, 하위 칸이 "등록된 회차가 없습니다"라고 **단정**했다 | "찾지 못함"과 "값 없음"을 한 표시로 접었다 | URL이 참조하는 외부 자원은 사라질 수 있고, 미해결 상태에서 하위 사실을 단정하면 거짓 진술이 된다는 것 |
| **F10** `[외부]` **헤더 전체선택이 해제되지 않았다.** 선택 불가 행이 섞인 페이지에서 전체 checkbox가 늘 "전체선택"으로만 동작 | 셀의 disabled 조건과 선택 가능 판정이 **서로 다른 소스**여서 테이블의 전체선택 상태가 false로 고정됐다 | 헤더 전체선택 상태가 **행의 선택 가능성에서 파생**되므로 그 판정의 소스가 하나여야 한다는 것 |
| **F11** `[외부]` 목록 행이 첫 렌더 프레임에 **얼어붙어 취소된 행의 체크박스가 잠겼고**, 체크박스 1클릭이 18개 행 780건 DOM 변이(89ms)를 냈다 | 행 memo 비교 키에 "선택 가능 여부"와 셀이 없었고, 네비게이션 훅이 매 렌더 새 객체를 반환해 컬럼 정의가 재생성됐다 | 선택 가능 여부가 **행 밖의 사실**에 의존하면 그 사실이 memo 키에 있어야 한다는 것 |
