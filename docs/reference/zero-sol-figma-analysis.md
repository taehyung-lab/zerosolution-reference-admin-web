...
27. `커뮤니티 > 게시판`(9.1) **카테고리 설정 팝업의 커밋 의미**. 원문 「게시판정보를 조회할 수 있다」는 팝업의 존재와
    ...
    - 2026-09-11 구현은 이 팝업과 카테고리 필드를 **만들지 않았다.** 빈 섹션을 그리면 미확인이 구현으로 보인다.

### 질문 28

28. `커뮤니티 > 게시판`(9.1) 등록의 **`피드백 설정` 세부 옵션**. 원장 14행(현 등록·수정 폼 행)의 Figma 관찰은 이름과
    `조회수` 가 함께 있다는 것뿐이고 Notion 원문에는 이 설정이 없다([시나리오 카드](scenarios/exhibition-promotion-community.md) 6절 3번과 같은 질문).
    - 답이 바꾸는 것: 등록·수정 폼의 필드 집합과 게시판 레코드의 값, 조회 화면의 표시 항목.
    - 2026-09-11 구현은 만들지 않았다.
...
29. `커뮤니티 > 게시판`(9.1) 등록·수정 폼에 **`읽기` 권한과 `사용상태` 입력이 있는가.** 원문 「게시판을 등록할 수 있다」의
    ...
    - 2026-09-11 구현은 원문이 열거한 세 필드(구분·게시판명·쓰기 권한)만 만들었다.
...
30. `커뮤니티 > 게시판`(9.1) **수정 화면의 필드 집합**. 원문은 조회의 `수정 버튼 → 클릭시, 수정 화면으로 이동` 만 적고
    ...
    - 2026-09-11 구현은 등록과 같은 세 필드를 공유했다. **관찰이 아니라 추론이다.**
...
31. `커뮤니티 > 게시판`(9.1) **조회 화면의 필드 집합·순서와 업데이트 내역의 열 구성**, 그리고 **삭제 완료 alert 문구**.
    ...
    - 2026-09-11 구현은 원장 12행이 레코드 값으로 열거한 9개를 그 순서로 두고 업데이트 내역은 공용 3열을 썼다.
      삭제는 확인까지만 만들고 완료 alert 는 만들지 않았다(API 도 없다).
...
32. 등록·수정 폼에서 쓰기 권한을 `회원등급`으로 고르면 하위 등급 입력이 필요한가. `notion/09-community.md` 39행(출처 절 「게시판 리스트를 조회할 수 있다」 — 검색 영역)은 `회원등급 : 해당 회원 등급 이상만 가능`이라 적고, 등록 절 42행은 등급 조건을 적지 않는다. 검색 영역의 [질문 23](#질문-23)은 필터 모양만 다룬다. 이 질문은 검색 절의 조건을 폼에 적용하는 것이 옳은지까지 묻는다.
    ...
    - 2026-09-11 구현은 등급 없이 `MEMBER_GRADE`를 제출한다 — 답이 오면 필드를 추가한다(드릴 2 독립 검토가 찾은 누락).
