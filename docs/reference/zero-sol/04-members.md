...
| 4.1.1 | 결과 toolbar 우 | `선택▾` + `변경` (cascade: 계정 상태 > 일반회원 / 불량회원 > 활동제한 설정 checkbox 스페셜콘텐츠·1:1문의·입장제한설정) · `SMS` · `이메일` · `등록`(primary) | (대기) | 변경·SMS·이메일이 선택 행 대상 bulk인지, 권한 | 없음(bulk) / `Link` 등록 |
...
| 4.1.4 조회 | 섹션 내 tab+검색+mini table | 활동정보: tab(티켓인증·관람인증·채권함·입장기록) + 검색 input(placeholder "공연명, 예매번호, 좌석번호", ⓧ clear) + checkbox table + `선택삭제` + paging. 컬럼은 checkbox·인증일·공연명·회차·공연일시·예매번호·좌석번호. 빈 상태 문구 "인증 기록이 없습니다." / 검색 무결과 "일치하는 검색결과가 없습니다." | (대기) | tab별 컬럼 차이·삭제 권한 | 없음 |
...
| 화면 | surface | Figma 관찰 | Notion 동작·정책 | 미확인 | 현재 코드 |
| --- | --- | --- | --- | --- | --- |
| 4.1.4 회원 등록 | 단일 섹션 폼 | 섹션 헤더 "회원정보"(접힘 없음). 1열 반폭: 이메일*·비밀번호*·이름*·생년월일*(date)·휴대폰번호*. placeholder가 제약 문구("3~100자 내외", "영문 대/소문자+숫자+특수문자 중 3종류 이상, 8~20자 내외"). 하단 중앙 `저장`·`취소` | Notion: 필수 미입력 → 필드 강조, 유효 → 저장 확인 alert → 저장 완료 alert | 서버 필드 오류 표시 위치 | Managers form 참고; 취소 경로는 `useUnsavedChangesGuard.leave()` |
