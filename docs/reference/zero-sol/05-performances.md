...
|  |  | 5.1 | table | checkbox. 구분·공연유형·공연명·총회차·출연자(콤마 결합, 말줄임)·주최/기획·공연기간·사용 상태·미리보기(link 셀, 없으면 `-`)·등록일/최근업데이트일(2줄 셀, 정렬 아이콘) | (대기) | 미리보기 팝업 대상. 2줄 셀의 정렬 아이콘이 등록일·최근업데이트일 중 어느 키인지 | `DataTable`; 방향 기본 `desc`(2026-09-11). **미구현**: 인벤토리의 등록일 컬럼이 `performance-columns.tsx` `fields`(ticketKind…updatedAt)에 없어 기본 sortType `registeredAt` 의 첫 렌더 `aria-sort` 를 붙일 헤더가 없다(2026-09-11 실측) |
