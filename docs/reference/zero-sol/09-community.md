...
| 9.1 게시판 | table | checkbox 없음, `No.`. 유형·구분·게시판명·권한(쓰기/읽기)·게시물수·게시물(**link 셀 "조회"** → 게시물 목록 이동)·사용상태·등록일/최근업데이트일 | (대기) | — | `DataTable`(`interactiveSelector` 가 `a` 를 행 활성화에서 제외하므로 link 셀 동작함) |
