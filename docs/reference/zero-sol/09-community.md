...
| 9.1 게시판 | toolbar 우 | `등록`만 | **일괄변경 있음** — `선택▾` + `변경`. Figma frame 에 없는 컨트롤이며 [질문 18](../zero-sol-figma-analysis.md#질문-18) 이 2026-09-09 Notion 을 채택했다. 대상 값은 원문에도 없다 | — | — |
| 9.1 게시판 | table | checkbox 없음(Figma), `No.`. 유형·구분·게시판명·권한(쓰기/읽기)·게시물수·게시물(**link 셀 "조회"** → 게시물 목록 이동)·사용상태·등록일/최근업데이트일 | **행 선택 checkbox 있음** — [질문 18](../zero-sol-figma-analysis.md#질문-18) 의 같은 채택. Figma frame 에 없다 | — | `DataTable`(`interactiveSelector` 가 `a` 를 행 활성화에서 제외하므로 link 셀 동작함) |
| 9.1 게시판 (`129:59279` Case) | 정렬·cascade | cascade 없음. 정렬은 등록일·최근업데이트일·유형·구분·게시판명·권한·게시물수 | (대기) | — | — |
