...
모든 섹션 파일은 아래 열에서 고른다. `화면`부터 `현재 코드`까지 여섯 열은 모든 표가 쓰고, `id`·`종류`는 **승격된 표만** 갖는다 — 승격은 표 단위로 점진적이며 그 규칙은 [실행 안내](../../../scripts/agents/README.md#read-the-migrated-rows)가 소유한다. 승격되지 않은 표는 기계가 0행으로 읽으며, 그것이 빈 화면을 뜻하지는 않는다. 다만 구현을 `settled`로 닫으려면 그 화면의 표가 승격돼 있어야 한다 — 다음 작업의 부분(slice) 진입이 이 행을 기계로 읽기 때문이다(`prepare`가 거절한다).
