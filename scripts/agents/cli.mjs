...
import { describeContext } from './surface-context.mjs'
...
try {
  if (command === 'bundle') {
    ...
    throw new Error('Usage: node scripts/agents/cli.mjs context [surface-id] | bundle [bundle-id] | prepare|review <session-id> .ai-work/<task>/<file>.json | hook [event]')
  }
} catch (error) {
