...
| 3. Screens | Implement one bounded resource lifecycle, or an explicitly requested component/surface/logic slice, using the target’s design and policy. Close the screens and dialogs that the target evidence places in that lifecycle; do not expand to another resource merely because it shares a domain. | Observe UI and interactions, verify state ownership/shared boundaries/failure paths, return to the cause of a failure and update its existing owner. Compare a different second consumer before confirming a provisional abstraction. |
...
`TRANSPLANT_MANIFEST` in `seed.mjs` declares the material that travels with selected bundles: whole skills, their linked ADRs, conditional version decisions, runtime entrypoints, gates, configuration, test harness and i18n runtime. This is the tool’s payload, not a requirement to install all of it during document preparation.

- `--bundles a,b` selects an exact opt-in code/test closure and the copied `SEED_BUNDLES` / `SEED_BUNDLE_EXPORTS` catalog. Omitting it selects the code-owned Foundation set; the README does not duplicate those IDs. The source bundle query keeps observed consumer examples for review, while the staged target catalog removes them. Bundle selection does not narrow the manifest’s whole skills or gate/config files. Resolve their dependencies or choose a smaller manual document adaptation; do not leave pointers to unavailable owners.
- `templates` and `config` entries require merging. Other files copy only when absent; existing target files are also merge items. Source product feature/routes/generated code, domain translations, app translations and `src/test/workflows/` tests are excluded. Consumer examples stay outside the code/test closure and must be replaced or removed in the target catalog.
- Default stage creates an empty fact-writing shell and generated index at the target product paths. It preserves schema/evidence rules, not source facts, and marks target facts unresolved. `--with-ledger` instead copies the active source ledger unchanged and is excluded from the clean-product procedure above.
...
- Staged `AGENTS.md` uses product-repository mode and the target fact pointer. The checker and package draft default to target mode. Package script paths not carried by the payload, classified source vocabulary and unresolved links appear in `PENDING.md`; unclassified vocabulary fails staging. The file is review input, not proof that every policy dependency was detected.
