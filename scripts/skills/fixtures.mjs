...
export const CASES = [
  ...
  { id: 'list/재표현', request: '리스트에서 검색 조건을 바꾸면 주소창에도 남아 있어야 합니다.', expect: ['list-contract'], reject: ['api-wire'] },
  ...
  { id: 'collection/재표현', request: '폼 안에서 행을 추가·삭제하는 반복 입력을 만들어주세요.', expect: ['collection-contract'], reject: ['logic'] },
  ...
  { id: 'route/재표현', request: '주소로 바로 열었을 때 데이터가 먼저 로드되게 해주세요.', expect: ['route-composition'], reject: ['shared-ui'] },
  ...
  { id: 'screen-composition/재표현', request: '새 화면 하나를 route 와 feature 로 어떻게 나눠야 하나요.', expect: ['screen-composition'], reject: ['form-contract'] },
  ...
  { id: 'file/재표현', request: '첨부 내려받기가 큰 파일에서 끊깁니다.', expect: ['file-workflow'], reject: ['route-composition'] },
  ...
  { id: 'specialized/재표현', request: '통계 보드에서 축을 바꾸면 셀이 다시 계산돼야 합니다.', expect: ['specialized-screens'], reject: ['form-contract'] },
  ...
  { id: 'shared-ui/재표현', request: '이 다이얼로그를 두 화면이 같이 쓰게 공용으로 올려도 될까요.', expect: ['shared-ui', 'source-structure'], reject: ['server-state'] },
  ...
  { id: 'api-wire/재표현', request: '서버가 주는 모양이 OpenAPI 스냅샷과 다릅니다.', expect: ['api-wire'], reject: ['detail-contract'] },
  ...
  { id: 'server-state/재표현', request: '필터가 비었을 때는 조회를 아예 하지 않게 해주세요.', expect: ['server-state'], reject: ['product-evidence'] },
  ...
  { id: 'auth/재표현', request: '401 이 오면 어디서 처리하고 있나요.', expect: ['auth-session'], reject: ['collection-contract'] },
  ...
  { id: 'logic/재표현', request: '이 매퍼 함수의 타입 오류를 고쳐주세요.', expect: ['logic'], reject: ['form-contract'] },
  ...
  { id: 'source-structure/재표현', request: '화면에서 옆 화면 파일을 import 하고 있는데 괜찮나요.', expect: ['source-structure'], reject: ['list-contract'] },
  ...
  { id: 'product/재표현', request: '이 문구가 실제 제품 문구가 맞나요.', expect: ['product-evidence'], reject: ['source-structure'] },
