...
vi.mock("../../../api/useSignInMutation", () => ({
  useSignInMutation: () => ({ mutateAsync, isPending: false }),
}));
...
describe("LoginScreen", () => {
  ...
  it("returns to the guarded destination the auth guard preserved", async () => {
    ...
    await waitFor(() =>
      expect(navigate).toHaveBeenCalledWith({
        to: "/managers?periodType=UPDATED_AT",
      }),
    );
  });
  ...
  it("preserves values and shows the generic credential message for resultCode 2100", async () => {
    mutateAsync.mockRejectedValue(
      new ApiError({ kind: "business", message: "rejected", code: "2100" }),
    );
    renderScreen();
  ...
  it.each([
    ...
    "shows a non-credential message for %s failures",
    async (kind, expectedMessage) => {
      ...
      expect(
        screen.queryByText("아이디 또는 비밀번호를 확인해주세요"),
