 ...
 * `읽기` 권한·사용상태는 원문 등록 절에 없고(질문 29), 카테고리는 등록 절(원문 45행)에 있지만 팝업의 항목
 * 모양·커밋 의미가 미확인이며(질문 27), 피드백 설정은 옵션이 미확인이라(질문 28) 여기 없다. `회원등급` 을
 * 고를 때 하위 등급 입력이 필요한지도 미확인이라(질문 32) 등급 없이 제출한다.
 *
