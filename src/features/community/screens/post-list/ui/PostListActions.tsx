...
import {
  ...
  postBulkChangeValue,
  usePostListActions,
} from '../model/usePostListActions';

/**
 * Figma 9.2.1 결과 toolbar 우측: `선택 ▾` + `변경` · `등록`.
...
export function PostListActions({
  ...
  const actions = usePostListActions(selectedIds);
  ...
  return (
    <>
      <div className="flex flex-wrap items-center gap-2">
        <Select
          aria-label={t('post.result.bulkField')}
          className="w-52"
          placeholder={t('post.result.bulkPlaceholder')}
          value={actions.target === undefined ? null : postBulkChangeValue(actions.target)}
          options={postBulkChanges.map((change) => ({
            value: postBulkChangeValue(change),
            label: t(`post.values.status.${change.value}`),
          }))}
          onValueChange={(value) => actions.setTarget(parsePostBulkChange(value))}
        />
        <Button onClick={requestBulkChange}>{t('post.result.bulkChange')}</Button>
        <Button onClick={onCreate}>{t('post.result.create')}</Button>
      </div>
