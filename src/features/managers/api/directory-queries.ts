...
export function managerDirectoryQuery(
  locale: string,
  search: ManagerListSearch,
) {
  return queryOptions({
    queryKey: [...localizedQueryKey(locale, "managers", "directory"), search],
    queryFn: () =>
      Promise.resolve().then(() => readManagerDirectoryPage(search)),
  });
}
export function managerDirectoryDetailQuery(locale: string, id: string) {
  return queryOptions({
    queryKey: localizedQueryKey(locale, "managers", "directory-detail", id),
    queryFn: () =>
...
export function managerDirectoryTypeOptionsQuery(locale: string) {
  return queryOptions({
    ...
    queryFn: () =>
      Promise.resolve().then(() => readManagerDirectoryTypeOptions()),
    staleTime: Infinity,
    ...inlineProgress,
  });
}
...
export function managerDirectoryPermissionOptionsQuery(
  ...
  return queryOptions({
    ...
    queryFn: () =>
      Promise.resolve().then(() => readManagerDirectoryPermissionOptions(type)),
