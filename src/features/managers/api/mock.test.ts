...
import {
  managerListQuery, managerDetailQuery, managerEditDetailQuery,
  managerTypeOptionsQuery, managerAgencyOptionsQuery, managerPermissionOptionsQuery,
} from '@/features/managers/api/queries';
import { resolveManagerSearch } from '@/features/managers/list/model/search-schema';
...
describe('generated API with shared HTTP mocks', () => {
  it('connects list, detail, edit and dependent options through the actual query options', async () => {
    ...
    try {
      const list = await client.query(managerListQuery('ko', resolveManagerSearch({ periodType: 'CREATED_AT' })));
      ...
      const permissions = await client.query(managerPermissionOptionsQuery('ko', 'AGENCY'));
      expect(types.some((type) => type.id === edit.type?.id)).toBe(true);
      expect(agencies.some((agency) => agency.id === edit.agency?.id)).toBe(true);
      expect(permissions.map((permission) => permission.id)).toEqual([edit.permission?.id]);
    } finally { client.clear(); }
  });

  it('filters, sorts and paginates the same records, including an empty result', async () => {
    const page = await getList8({ sortType: 'ID', sortDirection: 'ASC', pageNo: 2, pageSize: 1 });
    expect(page).toMatchObject({ pageNo: 2, pageSize: 1, totalCount: 2, list: [{ id: 'mock-manager-2' }] });
  ...
  it('does not turn an unknown record into an empty successful detail', async () => {
    const client = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    try {
      await expect(client.query(managerDetailQuery('ko', 'missing'))).rejects.toMatchObject({ kind: 'not-found' });
    } finally { client.clear(); }
  });

  it('reports unsupported query behavior instead of returning misleading results', async () => {
    await expect(getList8({ agencyIds: [1] })).rejects.toMatchObject({ status: 501 });
    await expect(getList8({ pageSize: 0 })).rejects.toMatchObject({ status: 400 });
  });

  it('returns an explicit mock failure for an unimplemented write', async () => {
    const response = await fetch('http://localhost:3000/api/v1/managers', { method: 'POST' });
