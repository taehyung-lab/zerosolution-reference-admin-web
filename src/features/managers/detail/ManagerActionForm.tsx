...
import { managerPasswordSchema } from '../form/manager-form-schema';
import type { ManagerDetailActionRequest } from './manager-detail-actions';

type InputAction = 'reject' | 'password' | 'unlock' | 'reveal' | 'verifyWithdrawal';
...
export function ManagerActionForm({
  ...
  const form = useForm({
    ...
    validators: {
      onDynamic: schema,
      onBlur: ({ value }) => ({ fields: { passwordConfirm:
        (action === 'password' || action === 'unlock') && value.passwordConfirm !== '' && value.password !== value.passwordConfirm
          ? t('form.errors.passwordMismatch') : undefined } }),
    },
    onSubmitInvalid: ({ formApi }) => {
      const first = (['reason', 'password', 'passwordConfirm', 'operatorPassword'] as const).find(
        (name) => formApi.getFieldMeta(name)?.errors.length,
      );
      if (first)
        document
          .getElementById(formFieldControlId<typeof formApi.state.values>(formApi, first))
          ?.focus();
    },
    onSubmit: ({ value }) => {
        ...
        >
          {action === 'reject' || action === 'verifyWithdrawal' ? (
            <FormTextField
              ...
              name="reason"
              label={t(
                action === 'reject' ? 'actions.rejectionReason' : 'actions.withdrawalReason',
              )}
              required
            />
          ) : null}
