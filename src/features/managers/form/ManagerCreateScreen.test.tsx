...
const proceed = vi.fn();
const reset = vi.fn();
let blocker: { status: 'idle' } | { status: 'blocked'; proceed: () => void; reset: () => void } = {
  status: 'idle',
};
...
vi.mock('./useCreateManagerMutation', () => ({
...
vi.mock('../options/useManagerOptions', () => ({
  ...
  useManagerPermissionOptions: (type: string | undefined) => ({
    data: type === undefined ? undefined : [{ value: '1', label: '일반관리자' }],
    isError: false,
    refetch: () => undefined,
  }),
...
async function confirmSave() {
  ...
  expect(within(confirmDialog).getByRole('heading', { name: '알림' })).toBeInTheDocument();
  expect(within(confirmDialog).getByText('저장하시겠습니까?')).toBeInTheDocument();
...
describe('ManagerCreateScreen — 서버 오류 분류', () => {
  it('닫힌 섹션의 서버 필드 오류를 보존해 열고 알리고 첫 rejected field로 포커스한다', async () => {
    ...
    expect(screen.getAllByText('서버에서 사용할 수 없는 값입니다.')).toHaveLength(2);
...
describe('ManagerCreateScreen — 옵션 조회 상태', () => {
  it('유형 옵션이 실패하면 select 자리에 오류와 다시 시도를 보이고, 성공하면 select로 돌아온다', () => {
    ...
    expect(screen.getAllByRole('alert')[0]).toHaveTextContent('옵션을 불러오지 못했습니다.');
...
describe('ManagerCreateScreen — 요청 본문', () => {
  ...
  it('UI 전용 비밀번호 확인을 본문에 싣지 않는다', async () => {
    ...
    await waitFor(() => expect(navigate).toHaveBeenCalledWith({ to: '/managers' }));
  });
  ...
  it('유형을 기획사에서 되돌리면 이전에 고른 기획사가 본문에 남지 않는다', async () => {
    ...
    await chooseOptionIn(/^유형/, '내부담당자');
    await waitFor(() => expect(screen.queryByRole('combobox', { name: /^기획사/ })).toBeNull());
