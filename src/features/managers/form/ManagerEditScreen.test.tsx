...
vi.mock('./useUpdateManagerMutation', () => ({
...
vi.mock('../options/useManagerOptions', () => {
  const ready = <T,>(data: T) => ({ data, isError: false, refetch: () => undefined })
  return {
    ...
    useManagerPermissionOptions: (type: string | undefined) =>
      ready(type === undefined ? undefined : [{ value: '3', label: '일반관리자' }]),
    useManagerAgencyOptions: () => ready([{ value: '7', label: '부스터랩' }]),
  }
})

const renderScreen = () => render(<TestLocaleProvider><ManagerEditScreen managerId="manager-1" /></TestLocaleProvider>)
...
describe('ManagerEditScreen load states', () => {
  ...
  it('keeps other failures recoverable and displays safe trace metadata', () => {
    ...
    vi.mocked(useQuery).mockReturnValue({
      isError: true,
      error: new ApiError({ kind: 'contract', message: 'raw contract body', status: 200, requestId: 'req-edit' }),
      refetch,
    } as never)
    renderScreen()
    expect(screen.getByRole('alert')).not.toHaveTextContent('raw contract body')
...
describe('ManagerEditScreen 저장 경로', () => {
  beforeEach(() => {
    ...
    vi.mocked(useQuery).mockReturnValue({ isPending: false, isError: false, data: detail } as never)
  })
  afterEach(() => {
    navigate.mockReset()
  ...
  it('조회 응답을 폼 값으로 펼치고, 아이디는 읽기 전용 텍스트이며 비밀번호 필드는 없다', () => {
    ...
    expect(screen.getByRole('combobox', { name: /^유형/ })).toHaveTextContent('기획사')
    expect(screen.getByRole('combobox', { name: /^기획사/ })).toHaveTextContent('부스터랩')
  ...
  it('저장 → 확인은 아이디·비밀번호 없는 PUT 본문을 보내고, 완료 확인 뒤 상세로 이동한다', async () => {
    ...
    expect(body).toMatchObject({ name: '김수정', type: 'AGENCY', permissionId: 3, agencyId: 7 })
    ...
    await waitFor(() =>
      expect(navigate).toHaveBeenCalledWith({ to: '/managers/$managerId', params: { managerId: 'manager-1' } }),
    )
  })
})
