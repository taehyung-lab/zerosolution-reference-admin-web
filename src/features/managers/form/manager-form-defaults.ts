 ...
 */
export function toManagerEditDefaults(detail: ManagerEditDetail): ManagerEditInput {
  return {
    type: detail.type?.id ?? '',
    agencyId: detail.agency?.id === undefined ? '' : String(detail.agency.id),
    permissionId: detail.permission?.id === undefined ? '' : String(detail.permission.id),
    name: detail.name ?? '',
    phone: detail.phone ?? '',
    email: detail.email ?? '',
