...
describe('create/edit 스키마 비대칭', () => {
  ...
  it('product types are required strings, not rehearsal enums or agency policies', () => {
    ...
    expect(managerCreateSchema.safeParse({ ...validCreate, type: 'PRODUCT_CUSTOM' }).success).toBe(true);
...
describe('요청 mapper 는 UI 전용 값을 본문에 싣지 않는다', () => {
  it('keeps rehearsal enum and required agency checks at the request boundary', () => {
    expect(() => toManagerCreateRequest({ ...validCreate, type: 'PRODUCT_CUSTOM' })).toThrow();
    expect(() => toManagerUpdateRequest({ ...validEdit, type: 'PRODUCT_CUSTOM' })).toThrow();
    expect(() => toManagerCreateRequest({ ...validCreate, type: 'AGENCY', agencyId: '' })).toThrow();
    expect(() => toManagerUpdateRequest({ ...validEdit, type: 'AGENCY', agencyId: 'invalid' })).toThrow();
    expect(toManagerCreateRequest({ ...validCreate, type: 'AGENCY', agencyId: '7' }).agencyId).toBe(7);
  });
