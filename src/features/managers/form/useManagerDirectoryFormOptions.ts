...
export function useManagerDirectoryFormOptions(type: string): ManagerFormOptions {
  const { locale } = useLocale();
  const typeSelected = type !== '';
  const types = useQuery(managerDirectoryTypeOptionsQuery(locale));
  const permissions = useQuery(
    managerDirectoryPermissionOptionsQuery(locale, typeSelected ? { type } : undefined),
  );
