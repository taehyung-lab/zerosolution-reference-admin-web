...
import { managerListSearchSchema, type ManagerListSearch } from './model/manager-list-search';
...
vi.mock(import('../fixtures/directory-options'), async (importOriginal) => {
  const actual = await importOriginal();
  return {
    ...actual,
    readManagerDirectoryTypeOptions: vi.fn(actual.readManagerDirectoryTypeOptions),
    readManagerDirectoryPermissionOptions: vi.fn(actual.readManagerDirectoryPermissionOptions),
  };
});
...
describe('manager product search input', () => {
  ...
  it('validates selection and confirms only eligible product-state rows', async () => {
    ...
    fireEvent.click(await screen.findByRole('checkbox', { name: '현재 페이지 전체 선택' }));
  ...
  it('보기 정렬 목록과 정렬 가능한 헤더가 같은 집합이고 활성 컬럼 하나만 aria-sort를 갖는다', async () => {
    ...
    expect(within(activeHeaders()[0]!).getByRole('button')).toHaveAccessibleName('가입일');

    fireEvent.click(within(table).getByRole('button', { name: '이메일' }));
    await waitFor(() =>
      expect(within(activeHeaders()[0]!).getByRole('button')).toHaveAccessibleName('이메일'),
    );
    expect(activeHeaders()).toHaveLength(1);
  });
});
...
describe('manager product filter option queries', () => {
  it('옵션이 도착하기 전에는 로딩을 보이고 도착하면 옵션을 보인다', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('combobox', { name: '권한' })).toBeEnabled());
    expect(screen.getByRole('checkbox', { name: 'Example type' })).toBeInTheDocument();
  });
  ...
  it('옵션 조회 실패를 빈 선택지로 숨기지 않고 필드별 재시도로 복구한다', async () => {
    ...
    expect(await screen.findByRole('alert', { name: '권한' })).toHaveTextContent(
      '옵션을 불러오지 못했습니다.',
    );
    expect(screen.queryByRole('combobox', { name: '권한' })).toBeNull();
