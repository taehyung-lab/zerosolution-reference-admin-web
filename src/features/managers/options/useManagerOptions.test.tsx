...
vi.mock('../api/queries', () => ({
  ...
  managerPermissionOptionsQuery: (locale: string, type: string | undefined) => ({
...
describe('manager option hooks', () => {
  it('maps supported manager types for select consumers', async () => {
    ...
    await waitFor(() =>
      expect(result.current.data).toEqual([{ value: 'INTERNAL', label: '내부담당자' }]),
    );
  });
  ...
  it('maps numeric ids and omits agency options without an id', async () => {
    ...
    await waitFor(() => expect(result.current.data).toEqual([{ value: '7', label: '부스터랩' }]));
  });

  it('does not fetch permissions until a manager type is selected', async () => {
    optionRequests.permissions.mockResolvedValue([{ id: 1, name: '일반관리자' }]);
    const client = new QueryClient({
      defaultOptions: { queries: { retry: false } },
    });
    const { result, rerender } = renderHook(
      ({ type }: { type: 'INTERNAL' | undefined }) => useManagerPermissionOptions(type),
      {
    ...
    await waitFor(() => expect(result.current.data).toEqual([{ value: '1', label: '일반관리자' }]));
  });
});
