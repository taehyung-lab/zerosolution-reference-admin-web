...
describe("manager detail input boundaries", () => {
  ...
  it.each(["active", "locked"] as const)(
    "reports initial confirmation blur mismatch and accepts one corrected submit: %s",
    async (status) => {
      const onActionRequest = setup(status);
      fireEvent.click(
        screen.getByRole("button", {
          name: status === "active" ? "비밀번호 변경" : "잠금해제",
        }),
      );
  ...
  it("validates unlock password and confirmation without fabricating activation", async () => {
    ...
    expect(
      screen.getByRole("dialog", { name: "잠금해제" }),
    ).toBeInTheDocument();
  });
  ...
  it("stops withdrawal at operator verification and retains values", async () => {
    ...
    expect(screen.getByLabelText("탈퇴 사유*")).toHaveValue(
