...
import {
  managerCreateDefaults,
  toManagerEditDefaults,
} from "./manager-form-defaults";
import {
  toManagerCreateRequest,
  toManagerUpdateRequest,
} from "./manager-form-request";
import {
  managerCreateSchema,
  managerEditSchema,
...
describe("create/edit 스키마 비대칭", () => {
  it("accepts the product name and phone bounds instead of the rehearsal patterns", () => {
    for (const name of ["김", "やまだ", "王小明", "Jane9"]) {
      expect(
        managerCreateSchema.safeParse({
          ...validCreate,
          name,
  ...
  it.each(["Abc!8275", "Safe!111", "Safe!321"])(
    "rejects repeated or sequential password characters: %s",
    (password) => {
      expect(
        managerCreateSchema.safeParse({
          ...validCreate,
  ...
  it("등록 스키마만 아이디와 비밀번호를 요구한다", () => {
    ...
    const issues =
      managerCreateSchema.safeParse(withoutCredentials).error?.issues ?? [];
  ...
  it("product types are required strings, not rehearsal enums or agency policies", () => {
    const asAgency = { ...validCreate, type: "AGENCY" as const };
    expect(managerCreateSchema.safeParse(asAgency).success).toBe(true);
    expect(
      managerCreateSchema.safeParse({ ...validCreate, type: "PRODUCT_CUSTOM" })
        .success,
    ).toBe(true);
    expect(
      managerCreateSchema.safeParse({ ...asAgency, agencyId: "7" }).success,
    ).toBe(true);
    expect(managerCreateSchema.safeParse(validCreate).success).toBe(true);
  });

  it("유형을 고르지 않은 초기 값은 통과하지 않는다", () => {
    expect(managerCreateSchema.safeParse(managerCreateDefaults).success).toBe(
      false,
    );
  });
});
...
describe("요청 mapper 는 UI 전용 값을 본문에 싣지 않는다", () => {
  it("keeps rehearsal enum and required agency checks at the request boundary", () => {
    expect(() =>
      toManagerCreateRequest({ ...validCreate, type: "PRODUCT_CUSTOM" }),
    ).toThrow();
    expect(() =>
      toManagerUpdateRequest({ ...validEdit, type: "PRODUCT_CUSTOM" }),
    ).toThrow();
    expect(() =>
      toManagerCreateRequest({ ...validCreate, type: "AGENCY", agencyId: "" }),
    ).toThrow();
    expect(() =>
      toManagerUpdateRequest({
        ...validEdit,
    ...
    expect(
      toManagerCreateRequest({ ...validCreate, type: "AGENCY", agencyId: "7" })
        .agencyId,
    ).toBe(7);
  });
  ...
  it("빈 문자열 optional 필드는 보내지 않는다", () => {
    const values = managerCreateSchema.parse({
      ...validCreate,
      organization: "",
    });
