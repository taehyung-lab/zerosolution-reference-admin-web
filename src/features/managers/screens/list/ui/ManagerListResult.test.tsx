...
import { managerSortFields, managerSortTypes } from "../model/manager-sort";
import {
...
describe("ManagerListResult", () => {
  ...
  it.each(["awaiting", "rejected", "locked"] as const)(
    "alerts and preserves selection when the selected row is %s",
    (accountStatus) => {
      const onActionRequest = vi.fn();
      renderResult(
        <ResultHarness
      ...
      expect(screen.queryByRole("dialog")).not.toBeInTheDocument();
      expect(
        screen.getByRole("checkbox", { name: "waiting 선택" }),
      ).toBeChecked();
    },
  );
  it("shows only the register action before the first search", () => {
    ...
    expect(
      screen.queryByRole("combobox", { name: "보기" }),
  ...
  it("does not render Pagination for a single-page result", () => {
    renderResult(
      <ResultHarness value={data({ rows: [row("1")], total: 1 })} />,
    );
  ...
  it("renders feature-owned columns without inventing unsupported selection actions", () => {
    ...
    expect(
      screen.queryByRole("checkbox", { name: "전체 선택" }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("columnheader", { name: "최근접속일" }),
    ).toBeInTheDocument();
  ...
  it.each(managerSortTypes)(
    ...
    (type) => {
      ...
      const routeSearch = onSearchChange.mock
        .calls[0]?.[0] as ManagerRouteSearch;
      expect(toManagerListParams(resolveManagerSearch(routeSearch))).toEqual(
  ...
  it("marks only the active header with the direction glyph and aria-sort", () => {
    ...
    expect(
      screen
        .getAllByRole("columnheader")
        .filter((h) => h.querySelector('[aria-hidden="true"]')),
    ).toHaveLength(1);
    expect(screen.getByRole("combobox", { name: "정렬" })).toHaveTextContent(
      "이름",
    );
