...
export function withdrawnDetailQuery(locale: string, id: string) {
  return queryOptions({
    queryKey: localizedQueryKey(locale, "members", "withdrawn-detail", id),
    queryFn: () =>
...
export function memberActivityQuery(
  locale: string,
  memberId: string,
  ...
  return queryOptions({
    queryKey: [
      ...localizedQueryKey(locale, "members", "activity", memberId),
      search,
    ],
    queryFn: () =>
      Promise.resolve().then(() =>
        selectMemberActivityFixture(memberId, search),
      ),
    ...contentProgress,
  });
}
...
export function memberCounselRecordsQuery(locale: string, memberId: string) {
  return queryOptions({
    queryKey: localizedQueryKey(locale, "members", "counsel-records", memberId),
    queryFn: () => Promise.resolve().then(() => memberCounselFixtures),
    ...contentProgress,
  });
}
...
export function counselReissueInputQuery(locale: string) {
  return queryOptions({
    queryKey: localizedQueryKey(locale, "members", "counsel-reissue-input"),
    queryFn: () => Promise.resolve().then(() => reissueInputFixture()),
    ...inlineProgress,
  });
}
export function counselDetailQuery(locale: string, id: string | undefined) {
  return queryOptions({
    queryKey: [...localizedQueryKey(locale, "members", "counsel-detail"), id],
    enabled: id !== undefined,
    queryFn: () =>
      Promise.resolve().then(() =>
        required(id === undefined ? undefined : counselDetailFixture(id)),
      ),
    ...blockingProgress,
  });
}
