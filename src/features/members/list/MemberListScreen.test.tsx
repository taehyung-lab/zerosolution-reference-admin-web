...
describe('active member list screens', () => {
  ...
  it('keeps every keyword added against the same target', () => {
    ...
    expect(screen.getByRole('button', { name: /first@example.com/ })).toBeInTheDocument();
  ...
  it('keeps the committed sort and page size when the filter panel is submitted again', () => {
    ...
    const { rerender } = render(
      <AllMemberListScreen {...baseProps} search={{ periodType: 'joinedAt' }} onSearchChange={onSearchChange} />,
    );
    rerender(
      <AllMemberListScreen
        {...baseProps}
