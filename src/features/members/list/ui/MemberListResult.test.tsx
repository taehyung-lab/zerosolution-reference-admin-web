...
import type { MemberListActionRequest, MemberListRow } from "../model/member-row";
...
describe("member list result", () => {
  it("renders masked values and adds the restriction column only for the flagged screen", () => {
    ...
    expect(
      screen.queryByRole("columnheader", { name: "활동제한" }),
    ...
    expect(
      screen.getByRole("columnheader", { name: "활동제한" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("cell", { name: "스페셜콘텐츠" }),
    ).toBeInTheDocument();
  });
  ...
  it.each(["notSearched", "loading", "error", "empty"] as const)(
    "keeps confirmation and its snapshot through %s",
    (state) => {
      const onActionRequest = vi.fn();
      const { rerender } = render(
        <Harness {...base} onActionRequest={onActionRequest} />,
      );
  ...
  it("preserves page-size and sort values when paging", () => {
    ...
    expect(screen.getByRole("combobox", { name: "보기" })).toHaveTextContent(
      "200",
    );
  ...
  it("clears selection when the committed view identity changes", () => {
    ...
    rerender(
      <Harness
        {...base}
        search={{ periodType: "joinedAt", sortType: "name" }}
      />,
    );
  ...
  it("does not restore a selected id after it leaves and later returns to the same result set", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("checkbox", { name: "박회원 선택" }),
      ).not.toBeChecked(),
    );
  ...
  it("keeps the flagged cascade feature-local and blocks an incomplete intent", () => {
    ...
    render(
      <Harness
        {...base}
        definition={memberListDefinitions.flagged}
        onActionRequest={onActionRequest}
      />,
    );
    fireEvent.click(screen.getByRole("checkbox", { name: "김회원 선택" }));
  ...
  it("keeps the flagged cascade through cancel and freezes it only on confirmation", () => {
    ...
    chooseTarget("불량회원");
    fireEvent.click(
      within(screen.getByRole("group", { name: "활동제한" })).getByRole(
        "checkbox",
        { name: "스페셜콘텐츠" },
      ),
    );
  ...
  it.each([
    ...
    "requests %s composition directly after selection without a confirmation step",
    (button, type) => {
      const onActionRequest = vi.fn();
