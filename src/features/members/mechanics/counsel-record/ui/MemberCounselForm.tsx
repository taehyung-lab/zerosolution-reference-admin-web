/**
 * 상담 한 건의 입력·날짜 검증·dirty 상태와 확인 callback을 관리하는 폼이다.
 * 상세 section과 팝업이 함께 사용한다. 실제 API에서도 검증/입력 역할은 유지하고 요청 DTO 변환은 계약에 맞춰 연결한다.
 */
...
import {
  memberCounselSchema,
  toCounselInput,
} from "../model/member-counsel-schema";
...
export function MemberCounselForm({
  ...
  readonly onSave: (values: MemberCounselInput) => void;
  ...
  const form = useForm({
    ...
    validators: {
      onDynamic: memberCounselSchema(today, requiredMessage, dateMessage),
    },
    onSubmit: ({ value }) =>
      onSave(toCounselInput(value, today, requiredMessage, dateMessage)),
    onSubmitInvalid: () => {
      element.current
    ...
    >
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <FormSelectField
          ...
          required
          options={memberCounselTypes.map((value) => ({
            value,
            label: t(`counsel.types.${value}`),
          }))}
        />
        <FormTextField
          form={form}
