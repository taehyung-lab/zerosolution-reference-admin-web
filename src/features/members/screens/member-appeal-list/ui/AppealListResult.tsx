...
import { appealSortKeys, type AppealRow, type AppealSortKey } from '@/features/members/model/member-records';
...
export function AppealListResult({
  ...
  readonly data: ListResultData<AppealRow>;
      ...
      <ResultToolbar
        left={
          <>
            ...
            <SortControl<AppealSortKey>
              label={t('result.sort')}
              value={view.sort.value}
              options={appealSortKeys.map((value) => ({ value, label: t(`fields.${value}`) }))}
