...
import type { readPerformancePage, performanceVenues } from '../fixtures/performances';

const { readPage } = vi.hoisted(() => ({ readPage: vi.fn<typeof readPerformancePage>() }));
vi.mock('../fixtures/performances', async (original) => ({
  ...await original<{ readPerformancePage: typeof readPerformancePage; performanceVenues: typeof performanceVenues }>(),
  readPerformancePage: readPage,
}));
const row = {
  id: 'show-1', ticketKind: 'day', performanceType: 'concert', title: 'Reference show',
  sessionCount: 2, performers: 'Performer', organizer: 'Organizer',
  period: '2026-09-01', venueId: 'venue-1', venueName: 'Venue A',
  seller: 'zero', registeredAt: '2026-09-01T00:00:00Z', updatedAt: '2026-09-02T00:00:00Z',
};
function setup(search: PerformanceRouteSearch = {}) {
  ...
  const view = (value: PerformanceRouteSearch) => (
    <QueryClientProvider client={client}><TestLocaleProvider>
      <PerformanceListScreen search={value} onSearchChange={onSearchChange} onActivate={onActivate}
        venues={[{ id: 'venue-1', name: 'Venue A' }, { id: 'venue-2', name: 'Venue B' }]} />
    </TestLocaleProvider></QueryClientProvider>
  );
  return { ...render(view(search)), view, onSearchChange, onActivate, client };
}
describe('performance list pre-request consumer', () => {
  beforeEach(() => readPage.mockReset().mockResolvedValue({ rows: [row], total: 101 }));
  it('returns to the Notion pre-search state on reset and resumes on submit', async () => {
    ...
    fireEvent.change(screen.getByRole('textbox', { name: '공연장 검색' }), { target: { value: 'Venue' } });
    ...
    fireEvent.click(screen.getByRole('button', { name: '초기화' }));
    expect(screen.getByRole('textbox', { name: '공연장 검색' })).toHaveValue('');
    expect(screen.getByText('검색 조건을 설정한 후 검색해 주세요.')).toBeInTheDocument();
  ...
  it('queries on entry without a search marker and renders a read-only reverse-numbered table', async () => {
    ...
    expect(screen.getByRole('combobox', { name: '기간 기준' })).toHaveTextContent('공연일');
    expect(readPage).toHaveBeenCalledWith(expect.objectContaining({ page: 1, pageSize: 100, sortType: 'registeredAt' }));
    ...
    expect(screen.queryByRole('button', { name: /^(변경|등록|SMS|이메일)$/ })).not.toBeInTheDocument();
  ...
  it('offers all source-defined categories and sellers, committing selections on search', async () => {
    ...
    await screen.findByRole('table');
    for (const label of ['콘서트', '뮤지컬', '전시', '페스티벌', '연극', '스포츠', '멤버십', '제로플러스', '멜론티켓', '티켓링크', '놀유니버스', '예스24', '기타']) {
  ...
  it('replaces the sole venue draft and commits it only on search with page reset', async () => {
    ...
    expect(readPage).toHaveBeenCalledTimes(1);
    fireEvent.click(screen.getByRole('button', { name: '검색' }));
    expect(onSearchChange).toHaveBeenLastCalledWith(expect.objectContaining({ venueId: 'venue-2', keywords: [{ field: 'title', value: 'pending' }] }));
    expect(onSearchChange.mock.calls.at(-1)![0].page).toBeUndefined();
    expect(onSearchChange.mock.calls.at(-1)![0]).not.toHaveProperty('venueKeyword');
    rerender(view(onSearchChange.mock.calls.at(-1)![0]));
