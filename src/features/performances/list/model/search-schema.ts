...
export const performanceSortTypes = ['registeredAt', 'updatedAt', 'period', 'ticketKind', 'performanceType', 'title', 'performers', 'organizer'] as const;
...
export const performanceSearchSchema = z.object({
  ...
  keywords: z.array(z.object({ field: z.enum(performanceKeywordFields), value: z.string().trim().min(1) })).optional().catch(undefined),
  ticketKinds: values,
  ...
  pageSize: z.coerce.number().pipe(z.union(standardPageSizeOptions.map((size) => z.literal(size)))).optional().catch(undefined),
}).transform((search) => {
  const next = compactSearchValues(search);
  if (next.startDateTime && next.endDateTime && Date.parse(next.startDateTime) > Date.parse(next.endDateTime)) {
    delete next.startDateTime;
    delete next.endDateTime;
  }
  if (next.periodType === defaultPerformancePeriodType) delete next.periodType;
...
export const performanceSearchPartition = {
  searched: 'filter',
  periodType: 'filter', startDateTime: 'filter', endDateTime: 'filter', keywords: 'filter',
  ticketKinds: 'filter', performanceTypes: 'filter', sellers: 'filter', venueId: 'filter',
  sortType: 'view', sortDirection: 'view', page: 'view', pageSize: 'view',
...
export const performanceSearchDefaults = {
  ...
  page: 1,
  pageSize: 100,
} as const satisfies Readonly<Record<keyof PerformanceRouteSearch, unknown>> & Partial<PerformanceRouteSearch>;
...
export type PerformanceSearch = ReturnType<typeof resolvePerformanceSearch>;
