...
import type { usePerformanceListData } from './usePerformanceListData';
import { useTranslation } from 'react-i18next';
import { performanceColumns } from '../ui/performance-columns';
import {
...
export function usePerformanceListResult(
  ...
  return {
    ...
    pageSize: {
      ...
      onValueChange: (pageSize: number) => change({ pageSize: pageSize as typeof resolved.pageSize, page: undefined }),
    },
    sort: {
      value: resolved.sortType,
      options: performanceSortTypes.map((value) => ({ value, label: t(`fields.${value}`) })),
      onValueChange: (sortType: typeof resolved.sortType) => change({ sortType, page: undefined }),
    },
    pagination: {
      page: data.page,
