import type { PerformanceVenueOptions } from '../model/usePerformanceVenues';
      ...
      <FilterField label={t('fields.performanceType')}>
        {({ labelId }) => (
          <CheckboxTree
            ariaLabelledby={labelId}
            emptyMeansAll
            nodes={['concert', 'musical', 'exhibition', 'festival', 'theatre', 'sports', 'membership'].map((value) => ({
              value,
              label: t(`options.${value}`),
            }))}
      ...
      <FilterField label={t('fields.seller')}>
        {({ labelId }) => (
          <CheckboxTree
            ariaLabelledby={labelId}
            emptyMeansAll
            nodes={['zero', 'melon', 'ticketlink', 'nol', 'yes24', 'other'].map((value) => ({
              value,
              label: t(`options.${value}`),
            }))}
