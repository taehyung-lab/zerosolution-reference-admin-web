...
export const printerPageSizes = standardPageSizeOptions;
type PrinterPageSize = (typeof printerPageSizes)[number];

const pageSizeSchema = z.coerce
  .number()
  .refine((value): value is PrinterPageSize =>
    (printerPageSizes as readonly number[]).includes(value),
  )
  .optional()
  .catch(undefined);
 ...
 * - sortDirection desc: 목록 공통 기본 방향(2026-09-11 사용자 확정). 활성 컬럼은 첫 렌더부터
 *   방향을 표시하므로 undefined 를 두지 않는다.
 ...
 * Figma 에 검색전 frame 이 없다 — 원장 README 진입 판독 규칙). 그래서 `searched` 표식도 없다.
 */
const fields = {
  ...
  keywords: {
    schema: recoverArray(
      z.object({
        field: z.enum(printerKeywordFields),
  ...
  statuses: {
    schema: recoverArrayItems(z.enum(printerStatuses)),
    defaultValue: [],
    kind: 'filter',
  },
  purposes: {
    schema: recoverArrayItems(z.enum(printerPurposes)),
    defaultValue: [],
    kind: 'filter',
  },
  usages: {
    schema: recoverArrayItems(z.enum(printerUsages)),
    defaultValue: [],
    kind: 'filter',
  },
} as const;

export const printerListSearchContract = defineSearchFields(fields);
export type PrinterListSearch = z.infer<typeof printerListSearchContract.schema>;

export function resolvePrinterListSearch(sparse: PrinterListSearch) {
  return resolveSearchDefaults(sparse, printerListSearchContract.defaults);
}

export type ResolvedPrinterListSearch = ReturnType<typeof resolvePrinterListSearch>;

/** canonical URL. 선언된 기본값과 빈 배열을 생략하고 기간은 양끝이 모두 유효할 때만 남긴다. */
export const printerListCanonicalSchema = printerListSearchContract.schema.transform((value) =>
  omitSearchDefaults(normalizeClosedInstantRange(value), printerListSearchContract.defaults),
);

/** 해소된 URL 값을 요청 입력으로 옮긴다. Query 키도 같은 객체를 쓴다. */
export function toPrinterListRequest(search: ResolvedPrinterListSearch): PrinterListRequest {
  return {
    page: search.page,
