...
import { requestManagerDetail } from "@/features/managers/detail/manager-detail-requests";
import {
  ManagerDetailContent,
} from "@/features/managers/detail/ManagerDetailScreen";
...
function ManagerDetailRoute() {
  ...
  if (record === undefined) return <DetailStateBoundary state={query.state} labels={{ error: t("detail.error"), notFound: t("detail.notFound") }} retryLabel={t("result.retry")} onRetry={() => { void query.retry(); }}>{null}</DetailStateBoundary>;
