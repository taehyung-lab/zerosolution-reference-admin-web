...
import { setAccessToken, clearAccessToken } from '@/api/http/credential';
...
it('환경 플래그 없이 제품 검색을 Query로 실행하고 초기화는 검색 전으로 돌아간다', async () => {
  ...
  const router = createAppRouter({ queryClient, history: createMemoryHistory({ initialEntries: ['/managers'] }) });
  ...
  expect(queryClient.getQueryCache().findAll({ queryKey: ['api', 'ko', 'managers', 'directory'] }).every(query => query.state.fetchStatus === 'idle')).toBe(true);
  fireEvent.click(within(form).getByRole('button', { name: '검색' }));
  expect(await screen.findByText('검색결과 : 105')).toBeVisible();
  expect(router.state.location.search).toMatchObject({ periodType: 'joinedAt' });
  fireEvent.click(within(form).getByRole('button', { name: '초기화' }));
  await waitFor(() => expect(screen.queryByRole('table')).not.toBeInTheDocument());
  expect(screen.getByText('검색 조건을 입력한 뒤 검색해 주세요.')).toBeVisible();
});
