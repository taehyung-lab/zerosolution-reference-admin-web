...
describe('active member list routes', () => {
  ...
  it('commits the search discriminator to the URL and resets back to pre-search', async () => {
    ...
    await waitFor(() => expect(router.state.location.search).toEqual({ searched: true }));
    fireEvent.click(screen.getByRole('button', { name: '초기화' }));
    await waitFor(() => expect(router.state.location.search).toEqual({}));
  });

  it('navigates registration to the member form and a missing member to the route 404', async () => {
    ...
    expect(await screen.findByRole('heading', { name: '회원 등록' })).toBeVisible();
    expect(screen.getByRole('textbox', { name: '이메일' })).toBeVisible();
  });

  it('opens a member detail from the loader cache', async () => {
    renderAt('/members/example-general');
    expect(await screen.findByRole('heading', { name: '회원 조회' })).toBeVisible();
  });
});
