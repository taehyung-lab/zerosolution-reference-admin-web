import type { DeepKeys, FieldComponent } from '@tanstack/react-form'
...
export function formFieldControlId<TValues>(name: DeepKeys<TValues>): string {
  return `form-field-${name.replaceAll('.', '-')}`
}

type FieldApi<TValues> = Parameters<NonNullable<Parameters<FieldForm<TValues>['Field']>[0]['children']>>[0]

interface FormFieldProps<TValues> {
  readonly form: FieldForm<TValues>
  readonly name: DeepKeys<TValues>
  ...
  readonly children: (field: FieldApi<TValues>, control: FieldControlProps, labelId: string) => ReactNode
}
...
export function FormField<TValues>({ form, name, label, required = false, description, labelTarget = 'control', children }: FormFieldProps<TValues>) {
  const inputId = formFieldControlId<TValues>(name)
