...
export function FormPermissionTreeField<TValues, TName extends DeepKeysOfType<TValues, string[]>>({ form, name, label, nodes, selectAllLabel, emptyMeansAll, required, description }: { readonly form: FieldForm<TValues>; readonly name: TName; readonly label: string; readonly nodes: readonly CheckboxTreeNode[]; readonly selectAllLabel: string; readonly emptyMeansAll?: boolean; readonly required?: boolean; readonly description?: string }) {
  return <FormField form={form} name={name} label={label} required={required} description={description}>{(field, control, labelId) => <CheckboxTree id={control.id} ariaDescribedby={control['aria-describedby']} ariaInvalid={control['aria-invalid']} ariaLabelledby={labelId} nodes={nodes} values={Array.isArray(field.state.value) ? field.state.value as string[] : []} onValueChange={(value) => field.handleChange(value as DeepValue<TValues, TName>)} onBlur={field.handleBlur} selectAllLabel={selectAllLabel} emptyMeansAll={emptyMeansAll} />}</FormField>
}
