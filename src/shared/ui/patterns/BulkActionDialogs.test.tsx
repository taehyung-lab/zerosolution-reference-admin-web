...
import { BulkActionDialogs, SelectionAlert, useBulkActionDialogs, useSelectionGate } from './BulkActionDialogs';
...
describe('useBulkActionDialogs', () => {
  it('lets one gate reject different actions without requiring confirmation', () => {
    ...
    function Harness({ count }: { readonly count: number }) {
      ...
      return <>
        <button onClick={() => { if (gate.requireSelection('Choose recipients')) run(); }}>Send</button>
        <button onClick={() => { if (gate.requireSelection('Choose rows')) run(); }}>Copy</button>
        <SelectionAlert controller={gate} />
      </>;
    }
    const { rerender } = render(<Harness count={0} />, { wrapper: ({ children }) => <I18nextProvider i18n={i18n}>{children}</I18nextProvider> });
  ...
  it('freezes values for confirmation and runs exactly those values', () => {
    ...
    const { result } = renderHook(() => useBulkActionDialogs({ run }));
  ...
  it('renders caller confirmation copy and discards values on cancel', () => {
    const run = vi.fn();
    function Harness() {
      const bulk = useBulkActionDialogs({ run });
      return <>
        <button onClick={() => bulk.requestConfirmation({ value: 1 })}>Change</button>
        <BulkActionDialogs controller={bulk} confirmDescription="Some rows cannot change. Continue?" />
      </>;
    }
    render(<I18nextProvider i18n={i18n}><Harness /></I18nextProvider>);
    fireEvent.click(screen.getByRole('button', { name: 'Change' }));
    expect(screen.getByRole('dialog')).toHaveTextContent('Some rows cannot change. Continue?');
